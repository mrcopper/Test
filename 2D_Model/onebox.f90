...
!set radial distance
!  rdist= 6   !in Rj
  rdist= 6+(.5/(RAD_GRID-1))*(radgrid-1)   !in Rj
...
!  fehot_const= .0022
  trans = 4.62963e-7
  net_source = source/10.0 ! ~6.3e6 fix FIX
...
subroutine rad_transport(n, nrg, dep)
  type(density)     ::n, nempty, ni
  type(nT)          ::nrg, nTempty, nrgi
  type(r_dep)       ::dep
...
!Improved Euler method
  ...
  n%sp   = n%sp  + .5 * (2.0*nin%sp  - ni%sp - n%sp )* dep%transport*dt
  n%s2p  = n%s2p + .5 * (2.0*nin%s2p - ni%s2p- n%s2p)* dep%transport*dt
  n%s3p  = n%s3p + .5 * (2.0*nin%s3p - ni%s3p- n%s3p)* dep%transport*dt
  n%op   = n%op  + .5 * (2.0*nin%op  - ni%op - n%op )* dep%transport*dt
  n%o2p  = n%o2p + .5 * (2.0*nin%o2p - ni%o2p- n%o2p)* dep%transport*dt

  nrg%sp   = nrg%sp  + .5 * (2.0*nTin%sp  - nrgi%sp - nrg%sp )* dep%transport*dt
  nrg%s2p  = nrg%s2p + .5 * (2.0*nTin%s2p - nrgi%s2p- nrg%s2p)* dep%transport*dt
  nrg%s3p  = nrg%s3p + .5 * (2.0*nTin%s3p - nrgi%s3p- nrg%s3p)* dep%transport*dt
  nrg%op   = nrg%op  + .5 * (2.0*nTin%op  - nrgi%op - nrg%op )* dep%transport*dt
  nrg%o2p  = nrg%o2p + .5 * (2.0*nTin%o2p - nrgi%o2p- nrg%o2p)* dep%transport*dt
...
!!!!!!!!!!!!!!!!!!!!DENSITY!!!!!!!!!!!!!!!!!!!!!
  ...
  if(outside<npes) call MPI_SEND(n%s, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nin%s, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(n%sp, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nin%sp, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(n%s2p, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nin%s2p, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(n%s3p, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nin%s3p, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(n%o, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nin%o, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(n%op, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nin%op, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(n%o2p, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nin%o2p, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

!!!!!!!!!!!!!!!!!!!!ENERGY!!!!!!!!!!!!!!!!!!!!!!
  if(outside<npes) call MPI_SEND(nrg%sp, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nTin%sp, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(nrg%s2p, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nTin%s2p, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(nrg%s3p, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nTin%s3p, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(nrg%op, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nTin%op, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(nrg%o2p, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside>0) call MPI_RECV(nTin%o2p, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)
